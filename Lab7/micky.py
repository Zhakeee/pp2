...
pygame.init() 
screen = pygame.display.set_mode([size, size]) # i did screen with size 800; 
pygame.display.set_caption("Mickey's Clock") # Clock's name 
...
# в этой функции я нахожу угол между каждый секунды и минуты 
...
while True: 
    ...
    for j in range(1, 13): 
        numbers(str(j), 80, polar_to_cartesian(clockRadius - 80, int(j) * 30)) 
    ...
    # second 
    theta = second * (360 / 60) 
    blitRotate(screen, leftHand, center, (leftHand.get_width() / 2 - 144.8, leftHand.get_height() / 2+2), theta - 87) 
